...
(function (window) {
  ...
  function renderKey( key ) {
    ...
    return '<div class="legend-item border1 legend-item-' + key + clickableClass + '" data-key="' + key + '">' +
             '<i class="leg-icon nav-item leg-icon-' + key + '"></i>' +
             //'<span class="legend-item-title"></span>' + (текст) //
           '</div>';
  }
        ...
        $container.on('nav_key:left nav_key:right nav_key:up nav_key:down', this.area_selector,
          function ( e ) {
            ...
            if ( fn ) {
              ...
              if ( !next.length && loop ) {
                next = $container.find(self.area_selector).filter(':')[last]();
              }
